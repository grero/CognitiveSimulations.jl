# TODO: Just define dummy functions here that will be fill in by the extension if loaded

function plot_network_output()
end

function plot_grid()
end

function show_task_schematic() end


function plot_trial() end

function plot_positions() end

function plot_connectivity_matrix() end

function plot_connectivity_matrix!() end

function plot_place_field() end

function plot_place_field2() end

function plot_view_tuning() end

function plot_view_and_hd_tuning() end

function plot_view_by_place_tuning() end

function plot_3d_snapshot() end

function plot_network_trials() end

function animate_task() end